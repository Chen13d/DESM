...
def main():
    ...
    if opt['read_version'] == "real-time":
        ...
        val_dataset = prepared_dataset(read_dir=read_dir_val, num_file=opt['num_test'], num_org=len(opt['category']), org_list=opt['category'], size=opt['size'], device=opt['device'], random_selection_flag=False, crop_flag=False, flip_flag=False)
