...
opt_path = 'options/train_DESM_IL_simulation.yml'
...
def main():
    ...
    net_main = DSCM_with_dataset(opt, in_channels=1, num_classes=len(opt['category']), model_name_G=opt['net_G']['model_decouple_name'], 
                          ...
                          scheduler_name=opt['train']['scheduler'], device=device, loss_factor_list=opt['net_G']['weight_decouple'], lr_G=opt['train']['lr_G'], lr_D=opt['train']['lr_D'])
    if opt['net_G']['weight_decouple'][5] > 0:
        net_main.net_D.train()
    
    combination_name = f"{opt['blur_factor']}" + f"_level_{opt['ellipse_level']}"
    ...
    train_loader, val_loader = gen_prepared_dataloader(read_dir_train=read_dir_train, read_dir_val=read_dir_val, num_file_train=opt['num_train'], 
                                                                    num_file_val=opt['num_val'], num_org=len(opt['category']), org_list=opt['category'], size=opt['size'], 
                                                                    batch_size=opt['train']['batch_size'], device=device, num_workers=opt['num_workers'], read_lifetime_flag=True)
    ...
    for epoch in range(opt['train']['epoches']):
        ...
        for iter in range(opt['train']['num_iter']):
            ...
            for batch_index, data in enumerate(train_loader):  
                Input, GT_DS, GT_D, GT_S, lifetime, sta = data
        ...
        if (epoch+1) % opt['train']['epoches_per_val'] == 0:
            with torch.no_grad():
                ...
                if (epoch+1) % opt['train']['epoches_per_save'] == 0:
                    toolbox.gen_validation_images_IL(data_list=[Input_list, GT_list, lifetime_list, sta_list])
