...
def evaluate_IL_synthetic(opt):
    ...
    with torch.no_grad():
        for batch_index, data in enumerate(val_loader):
            Input, GT_DS, GT_D, _, lifetime, sta = data
        ...
        if save_dir:
            ...
            if save_dir:
                ...
                for index in range(len(save_list)):
                    ...
                    tifffile.imwrite(os.path.join(save_dir, f"{index}_clustered_image.tif"), sorted_image)
                # save excel if needed
                df = pd.DataFrame({
                    "MAE": mae_list,
                    "SSIM": ssim_list,
                    "PCC": pearson_coef_list,
                })
    ...
    print(f"Average PCC: {np.mean(pearson_coef_list):.4f}")
